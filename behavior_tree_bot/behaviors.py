...
def attack_strongest_enemy_planet(state):
    ...
    strongest_enemy_planet = max(state.enemy_planets(),
                         key=lambda t: t.num_ships, default=None)

    if not my_strongest_planet or not strongest_enemy_planet:
        # No legal source or destination
        return False
    else:
        # (4) Send half the ships from my strongest planet to the weakest enemy planet.
        return issue_order(state, my_strongest_planet.ID, strongest_enemy_planet.ID, my_strongest_planet.num_ships % 10)
...
def spread_to_weakest_neutral_planet(state):
    ...
    if not my_strongest_planet or not weakest_neutral_planet:
        ...
        if dist < 15:
            return issue_order(state, my_strongest_planet.ID, weakest_neutral_planet.ID, weakest_neutral_planet.num_ships+10)
        else:
            return False
...
def attack_strongest_neutral_planet(state):
    ...
    if not my_strongest_planet or not target:
        ...
        if dist < 15:
            return issue_order(state, my_strongest_planet.ID, target.ID, target.num_ships+10)
        else:
            return False
        return False

def tactical_defense(state):
